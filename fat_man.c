...
static off_t    fixed_data_start = 0;   /* for fixed VHD or raw */

/* dynamic VHD */
static uint64_t dyn_bat_file_off;       /* BAT position in file */
static uint32_t dyn_max_bat;
static uint32_t dyn_block_size;         /* bytes per block */
static uint32_t dyn_sectors_per_block;
static uint32_t dyn_bitmap_sectors;     /* sectors used for sector bitmap */
static uint64_t dyn_virtual_size;
static uint32_t *bat = NULL;            /* in-memory BAT (host endian) */
static uint8_t  saved_footer[512];      /* footer we move as we grow */
...
/* Allocate a new block, preserving the VHD footer at the end */
static int bat_alloc_block(uint32_t block_idx) {
    if (block_idx >= dyn_max_bat || bat[block_idx] != VHD_BAT_FREE) return 0;

    struct stat st;
    fstat(img_fd, &st);
    off_t footer_off = st.st_size - 512;   /* current footer position */

    /* new block data goes where the footer currently is */
    ...
    if (!buf) { fprintf(stderr, "OOM allocating block\n"); return -1; }
    memset(buf, 0xFF, bitmap_bytes);  /* mark all sectors valid in bitmap */

    lseek(img_fd, footer_off, SEEK_SET);
    write(img_fd, buf, bitmap_bytes + data_bytes);
    free(buf);

    /* re-append footer at new end */
...
/* Translate LBA to file byte offset; allocate block if needed */
static off_t lba_to_off(LBA_t lba, int alloc) {
    ...
    if (bat[block_idx] == VHD_BAT_FREE) {
        if (!alloc) return -1;
        if (bat_alloc_block(block_idx) != 0) return -1;
    }

    return (off_t)bat[block_idx] * 512
         + (off_t)dyn_bitmap_sectors * 512
         + (off_t)sector_in_blk * 512;
}
...
static int open_image(const char *path) {
    ...
    /* read first 8 bytes to detect VHD */
    uint8_t hdr[8];
    lseek(img_fd, 0, SEEK_SET);
    if (read(img_fd, hdr, 8) != 8) return -1;
    ...
    /* save footer (it lives at end for both types) */
    struct stat st; fstat(img_fd, &st);
    ...
    if (vhd_type == VHD_TYPE_FIXED) {
        ...
        /* dynamic header offset is at footer+0x10 */
        ...
        if (!bat) { fprintf(stderr, "OOM\n"); return -1; }

        lseek(img_fd, (off_t)dyn_bat_file_off, SEEK_SET);
        int allocated = 0;
        for (uint32_t i = 0; i < dyn_max_bat; i++) {
            uint8_t b[4]; read(img_fd, b, 4);
            bat[i] = be32r(b);
            if (bat[i] != VHD_BAT_FREE) allocated++;
        }
...
DRESULT disk_read(BYTE pdrv, BYTE *buff, LBA_t sector, UINT count) {
    ...
    for (UINT i = 0; i < count; i++) {
        ...
        if (off < 0) { memset(buff + i*512, 0, 512); continue; }
        lseek(img_fd, off, SEEK_SET);
        if (read(img_fd, buff + i*512, 512) != 512) return RES_ERROR;
    }
    return RES_OK;
}
...
DRESULT disk_write(BYTE pdrv, const BYTE *buff, LBA_t sector, UINT count) {
    ...
    for (UINT i = 0; i < count; i++) {
        ...
        if (off < 0) return RES_ERROR;
        lseek(img_fd, off, SEEK_SET);
        if (write(img_fd, buff + i*512, 512) != 512) return RES_ERROR;
    }
    return RES_OK;
}
...
static int do_mount(void) {
    ...
    if (fr != FR_OK) { fprintf(stderr, "f_mount failed (%d)\n", fr); return -1; }
    return 0;
}

static void mkpath(const char *in, char *out, size_t sz) {
    if (in[0]=='/') snprintf(out, sz, "0:%s", in);
    else            snprintf(out, sz, "0:/%s", in);
}
...
#define CHECKFR(op, fr) do { if ((fr)!=FR_OK) { fprintf(stderr, "%s: %s\n", op, fr_str(fr)); return; } } while(0)
...
static void cmd_list(const char *path) {
    char fp[256]; mkpath(path, fp, sizeof(fp));
    DIR dir; FRESULT fr = f_opendir(&dir, fp);
    CHECKFR("f_opendir", fr);
    printf("\n%s\n%-6s %-40s %12s\n", path, "Type","Name","Size");
    printf("%-6s %-40s %12s\n","----","----","----");
    int files=0, dirs=0; FILINFO fno;
    while ((fr=f_readdir(&dir,&fno))==FR_OK && fno.fname[0]) {
        if (fno.fattrib & AM_DIR) { printf("%-6s %-40s %12s\n","[DIR]",fno.fname,"-"); dirs++; }
        else                      { printf("%-6s %-40s %12lu\n","",fno.fname,(unsigned long)fno.fsize); files++; }
    }
    f_closedir(&dir);
    if (!files && !dirs) printf("(empty)\n");
    printf("\n%d dirs, %d files\n\n", dirs, files);
}
...
static void cmd_mkdir(const char *path) {
    ...
    if (fr==FR_EXIST) { printf("Already exists: %s\n", path); return; }
    CHECKFR("f_mkdir", fr);
    printf("Created: %s\n", path);
}
...
static void cmd_import(const char *host, const char *dst) {
    ...
    if (!src) { perror(host); return; }
    char fp[256]; mkpath(dst, fp, sizeof(fp));
    FIL fil; FRESULT fr = f_open(&fil, fp, FA_CREATE_ALWAYS|FA_WRITE);
    if (fr!=FR_OK) { fprintf(stderr,"f_open: %s\n",fr_str(fr)); fclose(src); return; }
    uint8_t buf[4096]; size_t total=0, n;
    while ((n=fread(buf,1,sizeof(buf),src))>0) {
        UINT bw; fr=f_write(&fil,buf,(UINT)n,&bw);
        if (fr!=FR_OK||bw!=n) { fprintf(stderr,"f_write: %s\n",fr_str(fr)); break; }
        total+=n;
    }
    f_close(&fil); fclose(src);
    printf("Imported: %s -> %s (%zu bytes)\n", host, dst, total);
}

static void cmd_export(const char *src, const char *host) {
    char fp[256]; mkpath(src, fp, sizeof(fp));
    FIL fil; FRESULT fr = f_open(&fil, fp, FA_OPEN_EXISTING|FA_READ);
    if (fr!=FR_OK) { fprintf(stderr,"f_open: %s\n",fr_str(fr)); return; }
    FILE *dst = fopen(host, "wb");
    if (!dst) { perror(host); f_close(&fil); return; }
    uint8_t buf[4096]; size_t total=0; UINT br;
    while ((fr=f_read(&fil,buf,sizeof(buf),&br))==FR_OK && br>0)
        { fwrite(buf,1,br,dst); total+=br; }
    f_close(&fil); fclose(dst);
...
int main(int argc, char **argv) {
    ...
    if (!strcmp(cmd,"help")) { show_help(); return 0; }

    if (open_image(img) != 0) return 1;

    if (!strcmp(cmd,"format")) {
        cmd_format();
    } else {
        if (do_mount() != 0) { free(bat); close(img_fd); return 1; }
        if      (!strcmp(cmd,"info"))   cmd_info();
        else if (!strcmp(cmd,"list"))   cmd_list(argc>=4 ? argv[3] : "/");
        else if (!strcmp(cmd,"mkdir"))  { if(argc!=4) fputs("Usage: mkdir <path>\n",stderr);  else cmd_mkdir(argv[3]); }
        else if (!strcmp(cmd,"rmdir"))  { if(argc!=4) fputs("Usage: rmdir <path>\n",stderr);  else cmd_rmdir(argv[3]); }
        else if (!strcmp(cmd,"import")) { if(argc!=5) fputs("Usage: import <src> <dst>\n",stderr); else cmd_import(argv[3],argv[4]); }
        else if (!strcmp(cmd,"export")) { if(argc!=5) fputs("Usage: export <src> <dst>\n",stderr); else cmd_export(argv[3],argv[4]); }
        else if (!strcmp(cmd,"delete")) { if(argc!=4) fputs("Usage: delete <path>\n",stderr); else cmd_delete(argv[3]); }

...
static inline uint32_t nk_harp_blend_px(uint32_t dst, uint32_t src)
{
    ...
    uint32_t ia = 255 - a;
    return 0xFF000000 | (((sr * a + dr * ia) / 255) << 16) | (((sg * a + dg * ia) / 255) << 8) | ((sb * a + db * ia) / 255);
}
...
static void nk_harp_fill_rrect(nk_harp_t *nh, int x, int y, int w, int h, int r, uint32_t col)
{
    ...
    if (r < 1)
    {
        nk_harp_fill_rect(nh, x, y, w, h, col);
        return;
    }
    if (r > w / 2)
        r = w / 2;
...
static void nk_harp_stroke_line(nk_harp_t *nh, int x0, int y0, int x1, int y1, int t, uint32_t col)
{
    ...
    if (steps == 0)
    {
        nk_harp_fill_rect(nh, x0, y0, t, t, col);
        return;
    }
                                  ...
                                  int ax, int ay, int bx, int by, int cx, int cy, uint32_t col)
{
    ...
    for (int y = ymin; y <= ymax; y++)
    {
        ...
        if (xs <= xe)
            nk_harp_fill_rect(nh, xs, y, xe - xs + 1, 1, col);
    }
}
                              ...
                              uint32_t fg, uint32_t bg)
{
    ...
    if (sz < 8)
        sz = 8;
    if (y + sz + 2 <= nh->scy || y >= nh->scy + nh->sch)
        return;
    char tmp[512];
    int copy = len < 511 ? len : 511;
    memcpy(tmp, text, copy);
...
{
    int need_raster = full_clear || nh->first_frame || nh->had_event || nh->force_redraw;
    nh->first_frame = 0;
    nh->force_redraw = 0;
...
#define ZEN_TEXT_DIM nk_rgb(142, 142, 150)
    ...
    s->button.rounding = 7.0f;
    s->button.border = 1.0f;
    s->button.padding = nk_vec2(5, 3);
    ...
    s->contextual_button.rounding = 6.0f;
...
static void nk_harp_request_redraw(nk_harp_t *nh)
{
    if (nh)
        nh->force_redraw = 1;
}

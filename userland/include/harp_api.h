...
static inline harp_window_t *harp_open(const char *title, int x, int y, int w, int h)
{
    ...
    if (zen_shm_create(gw->shmname, (size_t)(w * h * 4) * 2, &si) < 0)
    {
        free(gw);
        return NULL;
    }
    gw->buf_base = (uint32_t *)si.addr;
    gw->cur_index = 0;
    gw->buf = gw->buf_base;
    memset(gw->buf_base, 0, (size_t)(w * h * 4) * 2);
...
{
    if (!gw || !gw->event_sock || !ev)
        return 0;
    if (socket_available(gw->event_sock) < sizeof(*ev))
        return 0;
    uint32_t got = 0;
    if (socket_read(gw->event_sock, ev, sizeof(*ev), &got) < 0 || got < sizeof(*ev))
        return 0;
    if (ev->type == HARP_EVENT_FOCUS)
        gw->focused = 1;
    else if (ev->type == HARP_EVENT_BLUR)
        gw->focused = 0;
    return 1;
}
...
static inline void harp_flush(harp_window_t *gw)
{
    ...
    gw->cur_index ^= 1;
    gw->buf = gw->buf_base + (size_t)gw->cur_index * (size_t)gw->w * (size_t)gw->h;
}
...
static inline void harp_flush_rect(harp_window_t *gw, int x, int y, int w, int h)
{
    ...
    msg.w = w;
    msg.h = h;
    msg.buf_index = (uint8_t)gw->cur_index;
    socket_write(gw->sock, &msg, sizeof(msg));
}

...
static inline ssize_t zread(int fd, void *buf, size_t count)
{
    ...
    int ret = (int)syscall4(11, (uint64_t)fd, (uint64_t)buf, count, (uint64_t)&bytes_read);
    if (ret < 0)
    {
        errno = 5;
        return -1;
...
static inline ssize_t zwrite(int fd, const void *buf, size_t count)
{
    ...
    int ret = (int)syscall3(12, (uint64_t)fd, (uint64_t)buf, count);
    if (ret < 0)
    {
        errno = 5;
        return -1;
...
static inline int zopen(const char *path, int flags, int mode)
{
    ...
    {
        write_mode = (flags & O_TRUNC) ? 2 : 1;
    }
    if (flags & O_CREAT)
        create(path);
    int fd = (int)syscall2(10, (uint64_t)path, (uint64_t)write_mode);
    if (fd < 0)
    {
        errno = 2;
        return -1;
...
static inline int feof(FILE *fp)
{
    ...
    off_t cur = zlseek(fd, 0, SEEK_CUR);
    off_t end = zlseek(fd, 0, SEEK_END);
    if (cur >= 0 && end >= 0 && cur < end)
        zlseek(fd, cur, SEEK_SET);
    return (cur >= end) ? 1 : 0;
}
    ...
    while (*fmt)
    {
        ...
        case 'X':
        {
            unsigned long long v = (lng == 2)   ? __builtin_va_arg(ap, unsigned long long)
                                   : (lng == 1) ? (unsigned long long)__builtin_va_arg(ap, unsigned long)
                                                : (unsigned long long)__builtin_va_arg(ap, unsigned int);
            tlen = _fmt_uint(tmp, v, 16, spec == 'X');
            tmp[tlen] = '\0';
            _fmtputs(tmp, width, left, pad, ctx);
            break;
        }

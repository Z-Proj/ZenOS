...
/*---------------------------------------------------------------------------
  ...
  DEFAULTFONTFILE is the filename of the font to be used if no other
    ...
    DEFAULTFONTDIR.
---------------------------------------------------------------------------*/
#define DEFAULTFONTDIR "/mnt/drv0/share/fonts"
...
/* ZenOS: no zip, no ioctl, no utf8, no TLF fonts */
/* ZFILE is just FILE* */
#define ZFILE FILE
#define Zopen(path,mode) fopen(path,mode)
#define Zclose(fp)       fclose(fp)
#define Zfgetc(fp)       fgetc(fp)
#define _Zgetc(fp)       fgetc(fp)
#define Zgetc(fp)        fgetc(fp)
#define Zungetc(c,fp)    ungetc(c,fp)
#define Zfgets(s,n,fp)   fgets(s,n,fp)
#define Zrewind(fp)      rewind(fp)

#include "fonts_embedded.c"
...
/* get_columns: not supported on ZenOS */
...
char *suffix;
{
  ...
  /* Try embedded fonts first - strip suffix for lookup */
  {
    char basename[64];
    int nlen = MYSTRLEN(name);
    int slen = MYSTRLEN(suffix);
    if (nlen < (int)sizeof(basename)) {
      strcpy(basename, name);
      /* strip suffix if present */
      if (slen > 0 && nlen > slen &&
          strcmp(basename + nlen - slen, suffix) == 0)
        basename[nlen - slen] = '\0';
      fontfile = open_embedded_font(basename);
  ...
  if (!hasdirsep(name)) {
    if (namelen + MYSTRLEN(name) + MYSTRLEN(suffix) + 2 < (int)sizeof(fontpath)) {
  ...
  if (MYSTRLEN(name) + MYSTRLEN(suffix) + 1 < (int)sizeof(fontpath)) {
...
static int get_columns(void) { return -1; }

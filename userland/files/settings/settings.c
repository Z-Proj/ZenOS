...
static int parse_hex_color(const char *s, uint32_t *out)
{
    ...
    if (digits == 6) { *out = 0xFF000000u | value; return 1; }
    if (digits == 8) { *out = value; return 1; }
    return 0;
}
...
typedef struct {
    ...
    int  len;
    int  enabled;
    int  is_comment; /* documentation line, not a toggleable command */
} boot_entry_t;
...
static void generate_thumbnail(const char *path)
{
    ...
    if (!resized) { stbi_image_free(img); return; }
...
static void load_wall(void)
{
    ...
    if (fp) {
        ...
        if (fgets(line, sizeof(line), fp)) {
            char *p = trim(line);
            if (*p) strncpy(wall_path, p, sizeof(wall_path) - 1);
        }
        fclose(fp);
    }
...
typedef struct {
    char path[128];  int path_len;
    char color[16];  int color_len;
    char label[8];   int label_len;
    char args[96];   int args_len;
} dock_entry_t;
...
static void render_dock(nk_harp_t *nh)
{
    ...
    if (nk_group_begin(ctx, "dock_list", NK_WINDOW_BORDER)) {
        ...
        for (int i = 0; i < dock_count; i++) {
            ...
            uint32_t swatch;
            if (!parse_hex_color(e->color, &swatch)) swatch = 0xFF3C3C50u;
            ...
            if (e->label_len > 3) e->label_len = 3;
...
static int power_confirm = 0; /* 0 none, 1 shutdown, 2 reboot */
...
static void render_power(nk_harp_t *nh)
{
    ...
    if (power_confirm == 0) {
        ...
        if (nk_button_label(ctx, "Shut Down")) power_confirm = 1;
        if (nk_button_label(ctx, "Reboot"))    power_confirm = 2;
    } else {
        nk_layout_row_dynamic(ctx, 20, 1);
        ...
        if (nk_button_label(ctx, "Confirm")) {
            if (power_confirm == 1) zen_shutdown();
            else zen_reboot();
        }
        if (nk_button_label(ctx, "Cancel")) power_confirm = 0;
    }
}
...
static void render_about(nk_harp_t *nh)
{
    ...
    utsname_t u; memset(&u, 0, sizeof(u)); uname(&u);
    fb_info_t fb; memset(&fb, 0, sizeof(fb)); zen_fbinfo(&fb);
    struct sysinfo si; memset(&si, 0, sizeof(si)); sysinfo(&si);
    int ncpu = get_nprocs();
    ...
    while (!nh->close_req) {
        ...
        if (nk_begin(&nh->ctx, "Settings",
            ...
            if (nk_group_begin(&nh->ctx, "content", NK_WINDOW_BORDER)) {
                switch (page) {
                case PAGE_BOOT:      render_boot(nh);      break;
                case PAGE_WALLPAPER: render_wallpaper(nh); break;
                case PAGE_DOCK:      render_dock(nh);      break;
                case PAGE_POWER:     render_power(nh);     break;
                case PAGE_ABOUT:     render_about(nh);     break;
                }
                nk_group_end(&nh->ctx);
            }
        }
